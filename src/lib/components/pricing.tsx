...
import { User } from '@supabase/supabase-js';
import type { ProductWithPrices, Subscription } from '@/lib/types/supabase/table.types';
import type { BillingIntervalType } from '@/lib/types/billing.types';
import { Heading } from '@/lib/components/ui/heading';
import { Text } from '@/lib/components/ui/text';
import { Card } from '@/lib/components/ui/card';
...
export default function Pricing({ user, products, subscription }: PricingProps) {
  ...
  function getProductsFor(products: any, type: BillingIntervalType) {
    return products.map((product: any) => {
      const plan = product.plans.find((p: any) => p.interval === type);
      if (!plan) return null;

      const price = plan.lineItems.find((l: any) => l.type === 'flat');
      if (!price) return null;
        ...
        >
          <Card className={`relative p-6 border-none ${product.isFeatured && 'outline outline-2 outline-primary'}`}>
            ...
            <div className="flex flex-col mt-4">
              {(product.features as [])?.map((feature: any) => {
                return (
                  <Text className="mt-2 inline-flex">
                    <span>
            ...
            <CheckoutButton subscription={subscription} plan={plan} />
          </Card>
        </div>
      );
    })
      ...
      <section>
        <Container size={10} className="py-20 lg:py-28">
          <div className="flex flex-col align-center">
            ...
            <Tabs className="flex flex-col justify-center mt-6" defaultValue="monthly">
              <TabsList className="mx-auto">
                <TabsTrigger className={`${(!paymentType.includes('recurring') || !intervals.includes('month')) ? 'hidden' : 'block'} px-4`} value="monthly">Monthly billing</TabsTrigger>
                <TabsTrigger className={`${(!paymentType.includes('recurring') || !intervals.includes('year')) ? 'hidden' : 'block'} px-4`} value="yearly">Yearly billing</TabsTrigger>
                <TabsTrigger className={`${!paymentType.includes('one_time') ? 'hidden' : 'block'} px-4`} value="lifetime">Life time</TabsTrigger>
              </TabsList>
              <TabsContent hidden={!paymentType.includes('recurring') && !intervals.includes('month')} value="monthly">
                <div className="mt-12 space-y-4 sm:mt-16 sm:space-y-0 flex flex-wrap justify-center gap-6 lg:max-w-4xl lg:mx-auto xl:max-w-none xl:mx-0">
                  {getProductsFor(billingConfig.products, 'month')}
                </div>
              </TabsContent>
              <TabsContent className={`${(!paymentType.includes('recurring') && !intervals.includes('year')) ? 'hidden' : 'block'}`} value="yearly">
                <div className="mt-12 space-y-4 sm:mt-16 sm:space-y-0 flex flex-wrap justify-center gap-6 lg:max-w-4xl lg:mx-auto xl:max-w-none xl:mx-0">
                  {getProductsFor(billingConfig.products, 'year')}
                </div>
              </TabsContent>
              <TabsContent hidden={!paymentType.includes('one_time')} value="lifetime">
                <div className="mt-12 space-y-4 sm:mt-16 sm:space-y-0 flex flex-wrap justify-center gap-6 lg:max-w-4xl lg:mx-auto xl:max-w-none xl:mx-0">
